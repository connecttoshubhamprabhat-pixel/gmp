...
####################################################### With the Tolerance Amount by 2 #####################################################
########### Below code is using for cron job and used on doc event of on_cancel, validate, on_update_after_submit, on_submit ###########
import frappe
from frappe.utils import flt


# Tolerance amount (in currency) — agar shortfall isse kam/equal hai,
# toh "Partially Paid" ke bajaye "Paid" set kar denge.
PIS_PAYMENT_TOLERANCE = 2
...
def update_submitted_pis_payment_status(doc=None, method=None):
    ...
    for purchase_order in purchase_orders:
        ...
        for pis in pis_records:
            ...
            elif shortfall <= PIS_PAYMENT_TOLERANCE:
                # Chhota sa shortfall (tolerance ke andar) hai,
                # isliye ise bhi "Paid" hi treat karenge.
                payment_status = "Paid"
                percentage_paid = 100
